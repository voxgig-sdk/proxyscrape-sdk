...
[MCP](https://modelcontextprotocol.io) server exposing the Proxyscrape SDK as
two agent tools — `proxyscrape_list` and `proxyscrape_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/proxyscrape-mcp)
make build

# 2. Provide credentials via the environment
export PROXYSCRAPE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user proxyscrape \
  -- /absolute/path/to/proxyscrape-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./proxyscrape-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// proxyscrape_list: first page of records
{ "entity": "proxy_list" }
{ "entity": "proxy_list", "query": { } }

// proxyscrape_load: one record by id
{ "entity": "proxy_list", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/proxyscrape-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user proxyscrape \
     -- "$PWD"/dist/*/proxyscrape-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `proxyscrape_list` and `proxyscrape_load` tools now appear
   in new sessions. Ask the agent to *"list proxy_list using proxyscrape"*
   and it calls `proxyscrape_list` with `{"entity":"proxy_list"}`.
...
```sh
export PROXYSCRAPE_APIKEY=sk_live_xxx            # API key
export PROXYSCRAPE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `proxyscrape_list` tool
...
### Call the `proxyscrape_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `proxyscrape_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `proxyscrape_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `PROXYSCRAPE_APIKEY` | API key sent with every request. |
| `PROXYSCRAPE_BASE` | Optional override of the API base URL. |
...
```sh
./proxyscrape-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"proxyscrape_load","arguments":{"entity":"proxy_list","query":{"id":1}}}}'
```
