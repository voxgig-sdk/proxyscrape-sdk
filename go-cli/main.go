// Command Proxyscrape-cli is a CLI + REPL for the Proxyscrape SDK,
...
const prompt = "proxyscrape"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "proxy_list"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      PROXYSCRAPE_APIKEY (api key), PROXYSCRAPE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: PROXYSCRAPE_APIKEY carries the API key and
	// PROXYSCRAPE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("PROXYSCRAPE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("PROXYSCRAPE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewProxyscrapeSDK(opts)
