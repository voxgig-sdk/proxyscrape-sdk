...
boru-driven command-line client **and** interactive REPL for the Proxyscrape
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/proxyscrape-cli)
make build

# 2. See usage (words, entities, env vars)
./proxyscrape-cli --help

# 3. Provide credentials once, via the environment
export PROXYSCRAPE_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./proxyscrape-cli list proxy_list

# 5. Override the API base URL for a single call
PROXYSCRAPE_BASE=https://api.example.com ./proxyscrape-cli list proxy_list

# 6. No arguments -> interactive REPL
./proxyscrape-cli
proxyscrape> list proxy_list
proxyscrape> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/proxyscrape-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/proxyscrape-cli list proxy_list
   ```
...
Run with no arguments to open a REPL (prompt `proxyscrape>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `proxy_list`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/proxyscrape-cli`. |
...
(`./proxyscrape-cli <expr>`) and interactively (the REPL), and expressions compose the
