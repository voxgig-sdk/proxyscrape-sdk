...
module ProxyscrapeUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = ProxyscrapeContext.new({}, nil)
    end
    op = ctx.op || ProxyscrapeOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || ProxyscrapeResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(ProxyscrapeError) ? err.msg : err.to_s
    ...
    sdk_err = ProxyscrapeError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(ProxyscrapeError)
