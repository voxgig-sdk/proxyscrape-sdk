...
// Proxyscrape SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class ProxyscrapeMakeError
{
    public static function call(?ProxyscrapeContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new ProxyscrapeContext([], null);
        }
        $op = $ctx->op ?? new ProxyscrapeOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new ProxyscrapeResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof ProxyscrapeError) ? $err->msg : (string)$err;
        $msg = "ProxyscrapeSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new ProxyscrapeError('', $msg, $ctx);
        ...
        if ($err instanceof ProxyscrapeError) {
