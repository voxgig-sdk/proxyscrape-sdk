...
{
    private static function utility(): ProxyscrapeUtility
    {
        return new ProxyscrapeUtility();
    }
    ...
    private static function ctx(array $over = []): ProxyscrapeContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new ProxyscrapeContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new ProxyscrapeOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof ProxyscrapeError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new ProxyscrapeOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = ProxyscrapeMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new ProxyscrapeOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = ProxyscrapeMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new ProxyscrapeOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = ProxyscrapeMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ProxyscrapeSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = ProxyscrapeMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new ProxyscrapeResponse([]),
            'result' => new ProxyscrapeResult([]),
        ]);
        [, $err] = ProxyscrapeMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new ProxyscrapeSpec([]),
            'response' => null,
            'result' => new ProxyscrapeResult([]),
        ]);
        [, $err] = ProxyscrapeMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new ProxyscrapeSpec(['step' => 's']),
            'response' => new ProxyscrapeResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new ProxyscrapeResult([]),
        ]);
        [, $err] = ProxyscrapeMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new ProxyscrapeSpec(['step' => 's']),
            'response' => new ProxyscrapeResponse(self::resp(200, ['v' => 1])),
            'result' => new ProxyscrapeResult([]),
        ]);
        [, $err] = ProxyscrapeMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new ProxyscrapeSpec(['step' => 's']),
            'response' => new ProxyscrapeResponse(self::resp(200, ['v' => 2])),
            'result' => new ProxyscrapeResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        ProxyscrapeMakeResponse::call($ctx);
    ...
    {
        $preset = new ProxyscrapeResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new ProxyscrapeSpec([]),
            'response' => new ProxyscrapeResponse([]),
            'result' => new ProxyscrapeResult([]),
        ...
        [$got, $err] = ProxyscrapeMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new ProxyscrapeResult([])]);
        [, $err] = ProxyscrapeMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new ProxyscrapeSpec([]), 'result' => null]);
        [, $err] = ProxyscrapeMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ProxyscrapeSpec(['step' => 's']),
            'result' => new ProxyscrapeResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new ProxyscrapeOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ProxyscrapeMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ProxyscrapeSpec(['step' => 's']),
            'result' => new ProxyscrapeResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new ProxyscrapeOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ProxyscrapeMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ProxyscrapeResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new ProxyscrapeSpec([]),
            'result' => new ProxyscrapeResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = ProxyscrapeMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ProxyscrapeMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new ProxyscrapeError('boom', 'boom');
        $utility->fetcher = function (ProxyscrapeContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ProxyscrapeSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = ProxyscrapeMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (ProxyscrapeContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = ProxyscrapeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(ProxyscrapeResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        ProxyscrapeMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (ProxyscrapeContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ProxyscrapeSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = ProxyscrapeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new ProxyscrapeResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new ProxyscrapeSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = ProxyscrapeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = ProxyscrapeMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = ProxyscrapeMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new ProxyscrapeResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, ProxyscrapeDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new ProxyscrapeResult(['ok' => false])]);
        $this->expectException(ProxyscrapeError::class);
        ProxyscrapeDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new ProxyscrapeResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', ProxyscrapeMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        ProxyscrapeMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new ProxyscrapeBaseFeature();
        $b = new ProxyscrapeBaseFeature();
        ProxyscrapeFeatureAdd::call($ctx, $a);
        ProxyscrapeFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): ProxyscrapeBaseFeature
    {
        $f = new ProxyscrapeBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        ProxyscrapeFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): ProxyscrapeContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new ProxyscrapeSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ProxyscrapePrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        ProxyscrapePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new ProxyscrapeResponse(['headers' => null]),
            'result' => new ProxyscrapeResult([]),
        ]);
        ProxyscrapeResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        ProxyscrapeResultBody::call($ctx);
