...
class FtCtrl extends ProxyscrapeControl
{
...
class FtClient
{
    ...
    private ProxyscrapeUtility $utility;

    public function __construct(ProxyscrapeUtility $utility, array $options)
    {
    ...
    public function get_utility(): ProxyscrapeUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ProxyscrapeContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ProxyscrapeContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ProxyscrapeContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ProxyscrapeConfig::make_feature($name);
    ...
    private static function buildUrl(ProxyscrapeSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ProxyscrapeError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ProxyscrapeResponse($fetched);
        }
    ...
    private function fail(ProxyscrapeContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ProxyscrapeContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
