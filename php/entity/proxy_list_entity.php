...
// Proxyscrape SDK ProxyList entity
...
class ProxyListEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new ProxyListEntity($this->_client, $opts);
    }

    /**
     * @param ProxyList|array $args ProxyList data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = ProxyscrapeHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return ProxyList|array The current ProxyList data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of ProxyList fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = ProxyscrapeHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of ProxyList fields).
     */
    ...
    /**
     * List ProxyList items matching the given filter.
     *
     * @param ProxyListListMatch|array|null $reqmatch Match filter (any subset
     *   of ProxyList fields) as an assoc-array; ProxyListListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return ProxyList[]|array A list of ProxyList items as assoc-arrays at
     *   the SDK boundary; throws ProxyscrapeError on failure (item-5 convention).
     */
