...
require_relative 'Proxyscrape_types'
...
class ProxyscrapeSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ProxyscrapeHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ProxyscrapeHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ProxyscrapeFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ProxyscrapeUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = ProxyscrapeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = ProxyscrapeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ProxyscrapeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = ProxyscrapeHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ProxyscrapeError.new(
        "graphql_error", "ProxyscrapeSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  # Canonical facade: client.ProxyList.list / client.ProxyList.load({ "id" => ... })
  def ProxyList(data = nil)
    require_relative 'entity/proxy_list_entity'
    ProxyListEntity.new(self, data)
  end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = ProxyscrapeSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
