...
module ProxyscrapeUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ProxyscrapeResponse.new({})
    ...
    if fetch_err
      ...
      response = ProxyscrapeResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ProxyscrapeResponse.new(fetched)
    else
