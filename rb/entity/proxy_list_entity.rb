...
class ProxyListEntity
  ...
  def make
    opts = @_entopts.dup
    ProxyListEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = ProxyscrapeHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [ProxyList, Hash] the current ProxyList data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = ProxyscrapeHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of ProxyList fields)
  def match_get
  ...
  #
  # @param reqmatch [ProxyListListMatch, Hash, nil] match filter (any subset of
  #   ProxyList fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<ProxyList>, Array] the matching ProxyList items; raises ProxyscrapeError on failure
