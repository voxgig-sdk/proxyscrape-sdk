...
const TEST_SEED = {"entity":{"proxy_list":{"test01":{"id":"test01"}}}}
...
function isRunnable(code: string): boolean {
  return (
    /new\s+ProxyscrapeSDK\b/.test(code) ||
    /\bProxyscrapeSDK\.test\b/.test(code) ||
    /\bclient\s*\./.test(code)
  )
}
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ProxyscrapeSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ProxyscrapeSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
