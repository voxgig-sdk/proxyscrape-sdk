...
import (
	...
	omni "github.com/voxgig-sdk/proxyscrape-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ProxyscrapeError); is {
				out := map[string]any{
					"name":    "ProxyscrapeError",
...
func omniCtx(arg any, client *sdk.ProxyscrapeSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ProxyscrapeSDK); is {
			client = live
