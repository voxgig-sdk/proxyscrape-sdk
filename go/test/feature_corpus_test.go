...
//
// NewProxyscrapeSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.ProxyscrapeSDK {
	res, _ := kase["res"].([]any)
	...
	return sdk.NewProxyscrapeSDK(opts)
}
...
func fcCandidates(client *sdk.ProxyscrapeSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.ProxyscrapeSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.ProxyscrapeSDK, name string) any {
