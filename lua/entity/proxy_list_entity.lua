...
local ProxyListEntity = {}
ProxyListEntity.__index = ProxyListEntity


function ProxyListEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ProxyListEntity)
  self._name = "proxy_list"
  self._client = client
...
function ProxyListEntity:get_name()
  return self._name
end


function ProxyListEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ProxyListEntity.new(self._client, opts)
end
...
function ProxyListEntity:mark_deleted()
  self._deleted = true
end


function ProxyListEntity:deleted()
  return true == self._deleted
end


function ProxyListEntity:data_set(args)
  if args ~= nil then
...
function ProxyListEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ProxyListEntity:match_set(args)
  if args ~= nil then
...
function ProxyListEntity:match_get()
...
function ProxyListEntity:stream(action, args, callopts)
...
---@param reqmatch ProxyListListMatch
---@param ctrl? table
---@return ProxyList[]
---@return string? err
function ProxyListEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function ProxyListEntity:_run_op(ctx, post_done)
  local utility = self._utility
