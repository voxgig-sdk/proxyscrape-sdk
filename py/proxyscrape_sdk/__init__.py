...
from proxyscrape_sdk.utility import register

# Load features
from proxyscrape_sdk.feature.base_feature import ProxyscrapeBaseFeature
from proxyscrape_sdk.features import _has_feature, _make_feature
...
class ProxyscrapeSDK:
    ...
    def get_utility(self):
        return ProxyscrapeUtility.copy(self._utility)
    ...
    def graphql(self, query, variables=None, ctrl=None):
        ...
        if isinstance(errors, list) and 0 < len(errors):
            ...
            res["ok"] = False
            res["err"] = Exception("ProxyscrapeSDK: graphql: " + str(msg))
            res["graphql"] = errors

        return res


    def ProxyList(self, data=None) -> "ProxyListEntity":
        """Entity factory: client.ProxyList().list() / client.ProxyList().load({"id": ...})."""
        from proxyscrape_sdk.entity.proxy_list_entity import ProxyListEntity
        return ProxyListEntity(self, data)



    @classmethod
    def test(cls, testopts=None, sdkopts=None) -> "ProxyscrapeSDK":
        if sdkopts is None:
            sdkopts = {}
