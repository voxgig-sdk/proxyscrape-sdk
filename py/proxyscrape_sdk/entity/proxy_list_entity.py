...
from proxyscrape_sdk.proxyscrape_types import (
    ProxyList,
    ProxyListListMatch,
)
...
class ProxyListEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ProxyListEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[ProxyList]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.ProxyList().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
