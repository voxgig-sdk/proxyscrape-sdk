...
class ProxyscrapeTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ProxyscrapeTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ProxyscrapeTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ProxyscrapeTestRunner.getenv("PROXYSCRAPE_TEST_LIVE")
        override = ProxyscrapeTestRunner.getenv("PROXYSCRAPE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ProxyscrapeTestRunner.getenv(key)
        ...
        explain = ProxyscrapeTestRunner.getenv("PROXYSCRAPE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["PROXYSCRAPE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if ProxyscrapeTestRunner._test_control is not None:
            return ProxyscrapeTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ProxyscrapeTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ProxyscrapeTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ProxyscrapeTestRunner.load_test_control()
...
def load_env_local():
    ProxyscrapeTestRunner.load_env_local()


def env_override(m):
    return ProxyscrapeTestRunner.env_override(m)


def entity_data(v):
    return ProxyscrapeTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ProxyscrapeTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ProxyscrapeTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ProxyscrapeTestRunner.load_test_control()


def live_client_options():
    return ProxyscrapeTestRunner.live_client_options()


def live_delay_ms():
    return ProxyscrapeTestRunner.live_delay_ms()
